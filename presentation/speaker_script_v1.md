# U-HAP — Presentation Speaker Script (VERSION 1 — original / most detailed)
...
Consider the paper's four policies — P1 through P4 — built from four atomic conditions:
*a1 "network is on-premise", a2 "time is business hours", a3 "department is engineering", a4 "clearance is top-secret."*
Several policies share whole sub-expressions: the gate **(a1 OR a2)** appears in both P1 and P2, and **(a2 AND a3)** appears in P2, P3, and P4.

In a naive tree those shared gates are rebuilt every time — seven gate instances plus four atoms, **11 nodes** total.
At request time you'd re-evaluate the same sub-expression again and again, even though the answer is identical.
`[PAUSE]`
**Hash consing** fixes this: before creating a node, check if this exact sub-expression already exists. If yes, reuse it.
The seven gate instances collapse to four unique gates — **8 nodes** instead of 11 — and each node is evaluated *at most once*, with its result memoized.
...
Here are all 21 references cited in the paper.
