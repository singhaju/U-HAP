...
\item KubeKeeper protects secrets against excessive permissions [11]; formal verification of access policies [12]
\end{itemize}
...
\begin{itemize}\tightitems
\item K8s RBAC [4]; KubeFence attack-surface hardening [13]; Zero Trust architecture [14]; PerfSPEC profiling [15]; compile-time policy optimization [16]
\end{itemize}
...
\begin{itemize}\tightitems
\item XACML [7]: expressive but heavyweight PDP, ill-suited to low-latency cloud-native; compiled-XACML indices [17] motivate our compile-time design
\item Graph / relationship-based AC [8,18]; Zanzibar [19] scales but needs \emph{runtime graph traversal}; ABAC$\to$RBAC conversion [20]
\end{itemize}
...
\scalebox{1.05}{%
\begin{tikzpicture}[
  gate/.style={draw=uhapNavy!55,fill=uhapGray,rounded corners=1pt,
               font=\tiny\bfseries,text=uhapNavyD,
               minimum width=0.64cm,minimum height=0.32cm},
  shar/.style={draw=uhapTeal,line width=0.6pt,fill=uhapTeal!18,rounded corners=1pt,
               font=\tiny\bfseries,text=uhapNavyD,
               minimum width=0.64cm,minimum height=0.32cm},
  atom/.style={draw=uhapNavy!40,fill=white,rounded corners=1pt,
               font=\tiny,text=uhapGrayD,minimum width=0.56cm,minimum height=0.30cm},
  satom/.style={draw=uhapTeal,fill=uhapTeal!12,rounded corners=1pt,
               font=\tiny,text=uhapNavyD,minimum width=0.56cm,minimum height=0.30cm},
  ptag/.style={font=\tiny\bfseries,text=uhapTeal},
  arr/.style={-{Stealth[scale=0.4]},uhapNavy!45,thin},
  tarr/.style={-{Stealth[scale=0.4]},uhapTeal!85,semithick}]

%% Title
\node[font=\tiny\bfseries,color=uhapNavyD,text width=4.6cm,align=center]
  at (2.05,4.05) {Hash-consed predicate DAG --- paper's P1--P4};

%% Layer-2 gates (roots of P2, P4)
\node[gate] (gP2) at (1.4,3.05) {AND};
\node[gate] (gP4) at (3.3,3.05) {AND};
\node[ptag] at (1.4,3.48) {P2};
\node[ptag] at (3.3,3.48) {P4};

%% Layer-1 gates (shared; also roots of P1, P3)
\node[shar] (gOR) at (0.85,1.7) {OR};
\node[shar] (g23) at (2.45,1.7) {AND};
\node[ptag,anchor=east] at (0.45,1.92) {P1};
\node[ptag,anchor=west] at (2.88,1.92) {P3};

%% Atoms (a2 shared)
\node[atom]  (a1) at (0.30,0.45) {a1};
\node[satom] (a2) at (1.55,0.45) {a2};
\node[atom]  (a3) at (2.70,0.45) {a3};
\node[atom]  (a4) at (3.80,0.45) {a4};

%% Edges (teal = into a shared node)
\draw[tarr] (gP2)--(gOR);
\draw[tarr] (gP2)--(g23);
\draw[tarr] (gP4)--(g23);
\draw[arr]  (gP4)--(a4);
...
%% Legend + atom key
\node[font=\fontsize{6}{6.5}\selectfont,color=uhapGrayD,text width=4.8cm,align=center]
  at (2.05,-0.08) {\textcolor{uhapTeal}{$\blacksquare$}~shared node \quad
   $a_1$\,net \ $a_2$\,time \ $a_3$\,dept \ $a_4$\,clearance};

%% Reduction caption
\node[font=\tiny\bfseries,color=uhapTeal,text width=4.9cm,align=center]
  at (2.05,-0.6) {7 gate instances $\to$ 4 unique gates:\ \ \textbf{11 $\to$ 8 nodes}};
\end{tikzpicture}%
}
\end{column}
%% ── Right: index table ──────────────────────────────────────────────
...
\item \textbf{+Cache:} ${\approx}$1.8\,\textmu s --- \textbf{${\sim}$14$\times$} speedup over SSO
...
\begin{callout}[fontupper=\scriptsize]{Key Results}
\begin{itemize}\tightitems
\item \textbf{ABAC:} hash-consed DAG → $\mathbf{16.9\times}$ at $k{=}320$; crossover $k{\approx}20$
\item \textbf{RBAC:} bit-vector AND → $\mathbf{5.6\times}$ at $k{=}320$; crossover $k{\approx}20$
\item \textbf{ACL:} hash-set → $\mathbf{5.0\times}$ at $k{=}320$; crossover $k{\approx}30$
...
% ════════════════════════════════════════════════════════════════════════════
\begin{frame}[t]{Experiment 3: Policy Update Latency vs.\ OPA~[21]}
\vspace{-2pt}
\begin{columns}[T,onlytextwidth]
%% Left: graph + key results
\begin{column}{0.55\textwidth}
...
\end{column}
\begin{column}{0.49\textwidth}
{\tiny\setlength{\parindent}{0pt}\setlength{\parskip}{2.0pt}%
\everypar{\hangindent=1.3em\hangafter=1}%
[11] M.\ Rostamipoor \emph{et al.}, ``KubeKeeper: Protecting Kubernetes Secrets Against Excessive Permissions,'' \emph{IEEE EuroS\&P}, 2025.\par
[12] A.\ Sissodiya \emph{et al.}, ``Formal Verification for Preventing Misconfigured Access Policies in Kubernetes,'' \emph{IEEE Access}, 13, 2025.\par
[13] C.\ Cesarano \& R.\ Natella, ``KubeFence: Security Hardening of the Kubernetes Attack Surface,'' \emph{IEEE/IFIP DSN}, 2025.\par
[14] R.\ Chandramouli \& Z.\ Butcher, ``A Zero Trust Architecture Model for Cloud-Native Applications,'' \emph{NIST SP 800-207A}, 2023.\par
[15] H.\ Kermabon-Bobinnec \emph{et al.}, ``PerfSPEC: Performance Profiling-Based Proactive Security Policy Enforcement for Containers,'' \emph{IEEE TDSC}, 22(2), 2025.\par
[16] S.\ Kern \emph{et al.}, ``Optimization of Access Control Policies,'' \emph{J.\ Inf.\ Secur.\ Appl.}, 70, 2022.\par
[17] A.\ X.\ Liu \emph{et al.}, ``Designing Fast and Scalable XACML Policy Evaluation Engines,'' \emph{IEEE Trans.\ Computers}, 60(12), 2011.\par
[18] L.\ Ma \emph{et al.}, ``Authorization Model of Attribute Access Control Based on Knowledge Graph,'' \emph{UbiSec}, 2024.\par
[19] R.\ Pang \emph{et al.}, ``Zanzibar: Google's Consistent, Global Authorization System,'' \emph{USENIX ATC}, 2019.\par
[20] M.\ Davari \& M.\ Zulkernine, ``Automatic Conversion of ABAC Policies for RBAC Systems,'' \emph{IEEE DSC}, 2023.\par
[21] Open Policy Agent, ``OPA --- Open Policy Agent,'' CNCF Graduated Project, 2024.\par
}
\end{column}
\end{columns}
\end{frame}
